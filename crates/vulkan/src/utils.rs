...
pub enum HandleError {
    #[error("The underlying handle cannot be represented using the types in this crate.")]
    NotSupported,
    #[error("The underlying handle is not available.")]
    Unavailable,
    #[error("An unknown error occurred.")]
    Unknown,
}

impl From<raw_window_handle::HandleError> for HandleError {
    fn from(e: raw_window_handle::HandleError) -> Self {
        match e {
            raw_window_handle::HandleError::NotSupported => HandleError::NotSupported,
            raw_window_handle::HandleError::Unavailable => HandleError::Unavailable,
            _ => HandleError::Unknown,
        }
    }
}
